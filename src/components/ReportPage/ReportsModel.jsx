...
const ReportCard = ({ title }) => (
  ...
  >
    <Row className="h-100">
      ...
      <Col xs={10}>
        <div className="fw-medium" style={{ fontSize: "18px", color: "#252525" }}>{title}</div>
        <div className="text-regular" style={{ fontSize: "14px", color: "#6B7582" }}>Shylaja Lab, Hyderabad, Telangana</div>
        <div className="text-regular" style={{ fontSize: "14px", color: "#6B7582" }}>25 Jan 2025, 241 KB</div>
      </Col>
    </Row>
  </Card>
);

const ReportsModel = ({ show, handleClose }) => {
  return (
    <>
      <style>
        {`
        ...
        <div style={{ padding: "40px 40px 0 40px" }}>
          ...
          {Object.entries(reports).map(([section, items]) => (
            <div key={section} className="mb-4">
              <h4 className="fw-medium mb-3" style={{ color: "#494949" }}>{section}</h4>
